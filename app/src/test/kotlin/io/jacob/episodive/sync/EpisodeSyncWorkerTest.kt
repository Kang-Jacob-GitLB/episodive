...
class EpisodeSyncWorkerTest {
    ...
    private fun createWorker(): EpisodeSyncWorker {
        return TestListenableWorkerBuilder<EpisodeSyncWorker>(context)
            .setWorkerFactory(TestWorkerFactory(syncNewEpisodesUseCase, notificationHelper))
            .build() as EpisodeSyncWorker
    }

    @Test
    fun `Given new episodes, when doWork, then returns success and shows notification`() = runTest {
    ...
    @Test
    fun `Given no new episodes, when doWork, then returns success without notification`() = runTest {
    ...
    @Test
    fun `Given sync failure, when doWork, then returns retry`() = runTest {
